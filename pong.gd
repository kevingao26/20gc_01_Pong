...
func _paddle_contact(left, right, ball_pos):
...
func _process(delta):
	...
	_paddle_contact(p1, p2, ball_pos)
	...
	# move left pad
	$p1.position = _move_pad($p1, delta, 1)
	$p2.position = _move_pad($p2, delta, 2)
